...
static int bh1750_emul_transfer(const struct emul *emul,
				struct i2c_msg *msgs,
				int num_msgs,
				int addr)
{
...
static int emul_rohm_bh1750_init(const struct emul *emul, const struct device *dev)
{
	struct bh1750_emul_data *data = emul->data;
...
#define BH1750_EMUL_INIT(inst)                          \
	...
	EMUL_DT_INST_DEFINE(inst, emul_rohm_bh1750_init, &data_##inst, &emul_##inst, &bh1750_emul_api, NULL);
